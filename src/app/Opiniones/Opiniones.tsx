    ...
    >
      <div className="mx-auto max-w-6xl">
        <div className="grid gap-9 lg:grid-cols-[1fr_auto] lg:items-end lg:gap-16">
          <div>
            <Reveal>
              <h2 className="text-[clamp(2.25rem,5.5vw,3.875rem)] leading-[1.05] text-arena-clara">
                Lo que dicen
              </h2>
            </Reveal>
            <Reveal delay={90}>
              <p className="mt-6 max-w-[35rem] text-base leading-[1.75] text-crema/80 text-pretty sm:text-[17px]">
          ...
          <Reveal delay={160}>
            <div className="flex flex-wrap items-center gap-4 border border-arena/30 px-7 py-4">
              ...
              <span className="font-display text-2xl leading-none text-arena-clara tabular-nums">
                {googleRating}
              </span>
        ...
        <div className="mt-16 gap-x-7 sm:mt-20 md:columns-2 xl:columns-3">
          {resenas.map((resena, index) => (
            <Reveal
              ...
              as="article"
              delay={index * 70}
              className="mb-7 flex break-inside-avoid flex-col border border-arena/25 bg-crema/4 p-7 sm:p-9"
            >
              <div className="flex items-start gap-4">
                <span className="flex h-11 w-11 flex-none items-center justify-center rounded-full border border-arena/45 text-[13px] font-semibold tracking-[0.08em] text-arena">
                  {resena.iniciales}
                </span>
                <span className="flex flex-col gap-1.5 pt-0.5">
                  <span className="font-display text-lg leading-[1.2] text-arena-clara">
                    {resena.nombre}
                  </span>
                  <span className="flex gap-1" aria-label="5 de 5 estrellas">
                    {[0, 1, 2, 3, 4].map((i) => (
                      <Star key={i} size={13} className="fill-arena text-arena" aria-hidden="true" />
                    ))}
                  </span>
                </span>
              </div>

              <p className="mt-6 font-display text-[17px] leading-[1.7] text-arena-clara/95 text-pretty">
                {resena.texto}
              </p>

              <span className="mt-6 flex flex-wrap gap-x-3 gap-y-1 border-t border-arena/20 pt-4 text-[10px] font-semibold tracking-[0.22em] text-arena/75 uppercase">
                <span>{resena.ticket}</span>
                <span aria-hidden="true">·</span>
                <span className="normal-case tracking-[0.06em] text-crema/50">
                  {resena.meta}
                </span>
              </span>
            </Reveal>
